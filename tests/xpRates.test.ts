...
import { observedMonsterXpRates, observedMonsterXpRatesOrEmpty } from "../src/bot/xpRates.js";
...
const buildFightLog = (
  ...
  ({
    content: { fight: { characters: [{ character_name: characterName, xp }], opponent } },
    cooldown: cooldownSeconds,
    type: "fight",
  }) as unknown as Log;

const buildOtherLog = (type: Log["type"]): Log => ({ content: {}, cooldown: 5, type }) as Log;
...
describe("observedMonsterXpRates", () => {
  it("averages xp/second across every fight found against each opponent", async () => {
    ...
    const result = await observedMonsterXpRates({ getCharacterLogs }, "Cartman");
...
describe("observedMonsterXpRatesOrEmpty", () => {
  it("degrades a log-fetch failure to an empty map instead of propagating it", async () => {
    ...
    const result = await observedMonsterXpRatesOrEmpty({ getCharacterLogs }, "Cartman");
