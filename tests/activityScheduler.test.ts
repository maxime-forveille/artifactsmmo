...
const buildSnapshot = (capturedAt = "2026-07-15T12:00:00.000Z"): CrewSnapshot => ({
  bank: [],
  capturedAt,
  characters: [],
});
...
const buildStarter = () => {
  ...
  const start = vi.fn(
    (state: OrchestratorState, assignment: ActivityAssignment<ExecutableActivity>) =>
...
describe("scheduleActivities", () => {
  it("plans from the current snapshot and starts the proposed Activity", async () => {
    ...
    expect((await scheduled.running[0]?.completion)?.event.type).toBe("completed");
  ...
  it("keeps a policy state change when no Activity is proposed", () => {
    ...
    const plan: ActivityPlanner<TestPlanError> = () => ok({ activities: [], state: plannedState });
    const { start } = buildStarter();

    const result = scheduleActivities(buildSnapshot(), state, plan, start);

    expect(result.isOk() && result.value).toEqual({ running: [], state: plannedState });
  ...
  it("starts several valid assignments against each preceding Reservation", () => {
    ...
    expect(result.isOk() && result.value.state.reservations).toEqual([stan, kyle]);
    expect(start).toHaveBeenNthCalledWith(1, state, stan);
    expect(start).toHaveBeenNthCalledWith(2, { goals: state.goals, reservations: [stan] }, kyle);
  });
  ...
  it("does not start anything when policy fails", () => {
    ...
    const result = scheduleActivities(buildSnapshot(), buildState(), plan, start);
