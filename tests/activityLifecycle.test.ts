...
const buildAssignment = (overrides: Partial<ActivityAssignment> = {}): ActivityAssignment => ({
...
describe("reserveStartedActivity", () => {
  ...
  it("allows different characters to work toward the same crew Goal", () => {
    ...
    expect(result.isOk() && result.value.reservations).toEqual([existing, assignment]);
  });
  ...
  it("rejects a second Reservation for the same character", () => {
    ...
    const result = reserveStartedActivity(state, buildAssignment({ goalId: "replenish-copper" }));
...
describe("finishActivity", () => {
  ...
  it("does not release a Reservation when the event names another Goal", () => {
    ...
    const result = finishActivity(state, buildTerminalEvent({ goalId: "another-goal" }));
    ...
    expect(error).toMatchObject({
      ...
      message: 'Character "Stan" is reserved for Goal "replenish-copper", not "another-goal"',
