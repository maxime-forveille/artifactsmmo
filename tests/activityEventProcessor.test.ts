...
const buildAssignment = (characterName: string, goalId: string): ActivityAssignment => ({
...
const buildState = (): OrchestratorState => ({
  ...
  reservations: [buildAssignment("Stan", "goal-stan"), buildAssignment("Kyle", "goal-kyle")],
});
...
describe("createActivityEventProcessor", () => {
  it("releases a completed Activity and refreshes the Crew Snapshot", async () => {
    ...
    const processor = createActivityEventProcessor<TestActivityError, TestSnapshotError>(
      buildState(),
      initialSnapshot,
      refreshSnapshot,
    );
    ...
    expect(result._unsafeUnwrap()).toEqual<ProcessedActivity<TestActivityError>>({
  ...
  it("preserves Blocker details for the next policy decision", async () => {
    ...
    expect(result.isOk() && "error" in result.value.outcome && result.value.outcome.error).toBe(
      blocker,
    );
    expect(processor.getState().goals).toEqual(buildState().goals);
  });
  ...
  it("serializes simultaneous outcomes against the latest state", async () => {
    ...
    const toSnapshotResult = (promise: Promise<CrewSnapshot>) =>
      ResultAsync.fromPromise(promise, () => new TestSnapshotError("refresh failed"));
    ...
    expect(refreshSnapshot).toHaveBeenCalledTimes(1);
    expect(processor.getState().reservations).toEqual([buildAssignment("Kyle", "goal-kyle")]);
  ...
  it("rejects an invalid event without refreshing or changing state", async () => {
    ...
    const refreshSnapshot = vi.fn(() => okAsync(buildSnapshot("2026-07-15T12:01:00.000Z")));
    const processor = createActivityEventProcessor<TestActivityError, TestSnapshotError>(
      initialState,
      buildSnapshot("2026-07-15T12:00:00.000Z"),
      refreshSnapshot,
    ...
    const result = await processor.process(buildOutcome("Butters", "goal-stan"));
