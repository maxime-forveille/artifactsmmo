...
describe("runCraftItemActivity", () => {
  it("moves to the recipe workshop and performs the requested craft", async () => {
    const { agent, client, craft, getItem, getMaps, moveTo } = buildDependencies();
  ...
  it("returns a typed Blocker without acquiring missing materials", async () => {
    ...
    const { agent, client, craft, getMaps, moveTo } = buildDependencies(character);
    ...
    expect(result._unsafeUnwrapErr()).toBeInstanceOf(MissingCraftingMaterialsError);
    expect(result._unsafeUnwrapErr()).toMatchObject({
      itemCode: "copper_dagger",
      message: 'Crafting "copper_dagger" needs materials that are not held by the character',
      missingMaterials: [
        {
          availableQuantity: 3,
          itemCode: "copper_bar",
  ...
  it("reports the exact profession level preventing the craft", async () => {
    ...
    expect(result._unsafeUnwrapErr()).toBeInstanceOf(InsufficientCraftingLevelError);
    expect(result._unsafeUnwrapErr()).toMatchObject({
      currentLevel: 4,
      itemCode: "copper_dagger",
      message:
  ...
  it("rejects an item without a crafting recipe", async () => {
    ...
    delete item.craft;
    const { agent, client, craft, getMaps } = buildDependencies(buildCharacter(), item);
  ...
  it.each([0, -1, 1.5])(
    ...
    async (quantity) => {
      ...
      expect(result._unsafeUnwrapErr()).toBeInstanceOf(InvalidCraftQuantityError);
      expect(result._unsafeUnwrapErr()).toMatchObject({
