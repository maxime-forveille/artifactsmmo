...
const buildMapPage = (data: Map[] = [{ ...({} as Map), map_id: BANK_MAP_ID }]): MapPage => ({
  data,
  page: 1,
  pages: 1,
  size: 50,
...
describe("runWithdrawItemActivity", () => {
  it("moves to the bank and withdraws the requested item", async () => {
    const { agent, client, getBankItems, getMaps, moveTo, withdrawItems } = buildDependencies();
    ...
    expect(moveTo).toHaveBeenCalledWith(BANK_MAP_ID);
    expect(withdrawItems).toHaveBeenCalledWith([{ code: "copper_dagger", quantity: 1 }]);
  });
  ...
  it.each([0, -1, 1.5])("rejects invalid quantity %s before reading the bank", async (quantity) => {
    ...
    expect(result._unsafeUnwrapErr()).toBeInstanceOf(InvalidWithdrawQuantityError);
  ...
  it("returns a typed Blocker when the inventory lacks room", async () => {
    ...
    const { agent, client, moveTo, withdrawItems } = buildDependencies(character);
    ...
    expect(result._unsafeUnwrapErr()).toBeInstanceOf(WithdrawInventoryFullError);
    expect(result._unsafeUnwrapErr()).toMatchObject({
      availableSpace: 0,
      itemCode: "copper_dagger",
      message: "Inventory has room for 0 item(s), but withdrawing 1x copper_dagger was requested",
  ...
  it("returns a location Blocker when no bank map exists", async () => {
    ...
    expect(result._unsafeUnwrapErr()).toEqual(new LocationNotFoundError("bank", "bank"));
    expect(moveTo).not.toHaveBeenCalled();
    expect(withdrawItems).not.toHaveBeenCalled();
  });
